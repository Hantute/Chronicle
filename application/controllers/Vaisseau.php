...
class Vaisseau extends CI_Controller {
    ...
    public function listeV() {
        ...
        if ($this->session->user) {
            ...
            // On charge le modèle 'Flotte_model'
            $this->load->model('Flotte_model');
            // On appelle la méthode Liste() du modèle, qui retourne le tableau résultat ici affecté à la variable $Fliste
            $Fliste = $this->Flotte_model->Liste();
            $aView["Fliste"] = $Fliste;

            // On charge le modèle 'Groupe_model'
            $this->load->model('Groupe_model');
            // On appelle la méthode liste() du modèle, qui retourne le tableau résultat ici affecté à la variable $Gliste
            $Gliste = $this->Groupe_model->liste();
            $aView["Gliste"] = $Gliste;

            // On charge le modèle 'vaisseau_model'
            $this->load->model('Vaisseau_model');
            // On appelle la méthode Vliste() du modèle, qui retourne le tableau résultat ici affecté à la variable $Vliste 
...
//******************************************************************************    
    ...
    public function ajoutV() {
        ...
        if ($this->session->user && $this->session->user->id_autorisation == "1") {
            ...
            $this->form_validation->set_rules('id_classe', 'classe', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('nom_vaisseau', 'nom', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('id_systeme', 'systeme', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('armement', 'armement', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('protection', 'protection', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('generateur', 'generateur', 'required', array('required' => 'Erreur dans le champs %s'));
...
//******************************************************************************        
    public function modificationV($id_vaisseau) {
        if ($this->session->user  && $this->session->user->id_autorisation == "1" ) {
            ...
            $this->form_validation->set_rules('nom_vaisseau', 'nom', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('chantier_de_construction', 'chantier', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('armement', 'armement', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('protection', 'protection', 'required', array('required' => 'Erreur dans le champs %s'));
            $this->form_validation->set_rules('generateur', 'generateur', 'required', array('required' => 'Erreur dans le champs %s'));
